...
final class RecipleaseAlamofireServiceTestCase: XCTestCase {
    ...
    override func setUp() {
        ...
        alamofireService = RecipeAlamofireService(manager: session)
    }
    ...
    func testGetRecipeShouldPostFailedCallBackIfNoData() {
        ...
        alamofireService?.getRecipe(ingredient: "tomatoes", callback: { (success, recipe) in
    ...
    func testGetRecipeShouldPostFailCallbackIfIncorrectResponse() {
        ...
        // When
        let expectation = XCTestExpectation(description: "wait for queue change.")
        alamofireService?.getRecipe(ingredient: "tomatoes", callback: { (failure, recipe) in
            // then
